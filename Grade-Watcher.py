...
# logger.setLevel(logging.DEBUG)  # 部署时 DEBUG 可改成 INFO
...
class User:
    def __init__(self, username, password, mail_sender, mail_sender_password, mail_sender_host,
        ...
        self.password_encrypt = self.encrypt(self.password) + "*" + hashlib.md5(self.password.encode("utf-8")).hexdigest()
...
class Login:
    ...
    def recognize_captcha(self):
        ...
        for i in range(self.retries + 10):
            try:
    ...
    def __call__(self):
        ...
        try:
            ...
            if match:
                ...
                raise ValueError("链接匹配错误")

            data = str(self.get(match_url))
            self.user.grade = show(data.replace('<html><body><p>', '').replace('</p></body></html>', ''))
            return
        ...
        for i in range(self.retries):
            ...
            try:
                ...
                with smtplib.SMTP_SSL(host=self.user.mail_sender_host, port=self.user.mail_sender_port) as smtp:
                    ...
                    smtp.sendmail(from_addr=self.user.mail_sender, to_addrs=self.user.mail_receiver.split(','),
                                  msg=msg.as_string())
                logger.info("邮件发送成功")
                return
    ...
    def start_monitoring(self, retries=20):
        ...
        for j in range(retries):
            ...
            try:
                while True:
                    self.fetcher()
                    if self.user.grade != current_grade:
                        current_grade = self.user.grade
                        logger.info("更新成功\n" + current_grade)
